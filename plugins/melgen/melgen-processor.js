// plugins/melgen/melgen-processor.js
//
// The AudioWorklet processor for MelGen.
//
// A melody generator with no audio: it fills the transport block the module
// reads, hands over incoming follow notes and conductor CCs, and posts back
// whatever the module generated. It is the same shape as the drumgen
// processor, which is the other jig:Abi2 generator with MIDI in both
// directions, and the same two rules apply. Every incoming event carries an
// absolute stream position and is applied in the quantum that contains it,
// compared by range and never by equality with a boundary. The queue is
// bounded, preallocated, and says how much it dropped.
//
...
// and tests/host/melgen.test.js fails if the three disagree in a name, a
// range or a default.

const PARAM_INDEX = Object.freeze({
  root_note: 0, scale: 1, channel: 2, length_beats: 3, phrase_bars: 4,
  subdivision: 5, period: 6, contour: 7, answer: 8, density: 9,
  register: 10, hold: 11, accent: 12, structure: 13, range: 14,
  leap: 15, rest: 16, cadence: 17, seed: 18, vary: 19,
  new: 20, notes: 21, rhythm: 22, follow: 23, color: 24, conductor_ch: 25
})
...
class MelgenProcessor extends AudioWorkletProcessor {
  static get parameterDescriptors () {
    return [
      { name: 'root_note', defaultValue: 60, minValue: 0, maxValue: 127, automationRate: 'k-rate' },
      { name: 'scale', defaultValue: 0, minValue: 0, maxValue: 22, automationRate: 'k-rate' },
      { name: 'channel', defaultValue: 1, minValue: 1, maxValue: 16, automationRate: 'k-rate' },
      { name: 'length_beats', defaultValue: 16, minValue: 4, maxValue: 64, automationRate: 'k-rate' },
      { name: 'phrase_bars', defaultValue: 2, minValue: 1, maxValue: 8, automationRate: 'k-rate' },
      { name: 'subdivision', defaultValue: 1, minValue: 0, maxValue: 3, automationRate: 'k-rate' },
      { name: 'period', defaultValue: 4, minValue: 0, maxValue: 5, automationRate: 'k-rate' },
      { name: 'contour', defaultValue: 4, minValue: 0, maxValue: 5, automationRate: 'k-rate' },
      { name: 'answer', defaultValue: 0, minValue: 0, maxValue: 5, automationRate: 'k-rate' },
      { name: 'density', defaultValue: 0.48, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'register', defaultValue: 1, minValue: 0, maxValue: 4, automationRate: 'k-rate' },
      { name: 'hold', defaultValue: 0.42, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'accent', defaultValue: 0.45, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'structure', defaultValue: 0.62, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'range', defaultValue: 0.45, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'leap', defaultValue: 0.28, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'rest', defaultValue: 0.24, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'cadence', defaultValue: 0.55, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'seed', defaultValue: 1, minValue: 1, maxValue: 65535, automationRate: 'k-rate' },
      { name: 'vary', defaultValue: 0, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'new', defaultValue: 0, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'notes', defaultValue: 0, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'rhythm', defaultValue: 0, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      ...
      { name: 'conductor_ch', defaultValue: 0, minValue: 0, maxValue: 16, automationRate: 'k-rate' }
    ]
  }
...
registerProcessor('melgen', MelgenProcessor)

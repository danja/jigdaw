// plugins/ground/ground-processor.js
//
// The AudioWorklet processor for Ground.
//
// A long-form bass generator with no audio: it fills the transport block the
// module reads, hands over incoming conductor CCs, and posts back whatever
// the module generated. It is the same shape as the drumgen processor, which
// is the other jig:Abi2 generator with MIDI in both directions, and the same
// two rules apply. Every incoming event carries an absolute stream position
// and is applied in the quantum that contains it, compared by range and
// never by equality with a boundary. The queue is bounded, preallocated, and
...
// and tests/host/ground.test.js fails if the three disagree in a name, a
// range or a default.

const PARAM_INDEX = Object.freeze({
  root_note: 0, scale: 1, style: 2, channel: 3, form_bars: 4, phrase_bars: 5,
  density: 6, motion: 7, tension: 8, cadence: 9, register: 10,
  register_arc: 11, sequence: 12, seed: 13, vary: 14,
  new_form: 15, new_phrase: 16, mutate_cell: 17, color: 18,
  note_length: 19, note_length_variation: 20, form_shape: 21,
  phrase_role_1: 22, phrase_role_2: 23, phrase_role_3: 24, phrase_role_4: 25,
  phrase_role_5: 26, phrase_role_6: 27, phrase_role_7: 28, phrase_role_8: 29,
  phrase_role_9: 30, phrase_role_10: 31, phrase_role_11: 32, phrase_role_12: 33,
  phrase_role_13: 34, phrase_role_14: 35, phrase_role_15: 36, phrase_role_16: 37,
  phrase_role_17: 38, phrase_role_18: 39, phrase_role_19: 40, phrase_role_20: 41,
  phrase_role_21: 42, phrase_role_22: 43, phrase_role_23: 44, phrase_role_24: 45,
  phrase_role_25: 46, phrase_role_26: 47, phrase_role_27: 48, phrase_role_28: 49,
  phrase_role_29: 50, phrase_role_30: 51, phrase_role_31: 52, phrase_role_32: 53,
  clamp: 54, conductor_ch: 55
})
...
class GroundProcessor extends AudioWorkletProcessor {
  static get parameterDescriptors () {
    const dial = (name, defaultValue, minValue, maxValue) =>
      ({ name, defaultValue, minValue, maxValue, automationRate: 'k-rate' })
    const descriptors = [
      dial('root_note', 36, 0, 127), dial('scale', 2, 0, 22),
      dial('style', 0, 0, 11), dial('channel', 1, 1, 16),
      dial('form_bars', 16, 8, 64), dial('phrase_bars', 4, 1, 12),
      dial('density', 0.45, 0, 1), dial('motion', 0.55, 0, 1),
      dial('tension', 0.45, 0, 1), dial('cadence', 0.5, 0, 1),
      dial('register', 1, 0, 3), dial('register_arc', 0.4, 0, 1),
      dial('sequence', 0.35, 0, 1), dial('seed', 1, 1, 65535),
      dial('vary', 0, 0, 1),
      dial('new_form', 0, 0, 1), dial('new_phrase', 0, 0, 1),
      dial('mutate_cell', 0, 0, 1), dial('color', 0, 0, 1),
      dial('note_length', 0.35, 0, 1), dial('note_length_variation', 0.35, 0, 1),
      dial('form_shape', 0, 0, 12)
    ]
    for (let role = 1; role <= 32; role++) descriptors.push(dial(`phrase_role_${role}`, 0, 0, 7))
    descriptors.push(dial('clamp', 12, 12, 48), dial('conductor_ch', 0, 0, 16))
    return descriptors
  }
...
registerProcessor('ground', GroundProcessor)

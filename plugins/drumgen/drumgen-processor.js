// plugins/drumgen/drumgen-processor.js
//
// The AudioWorklet processor for DrumGen.
//
// A MIDI generator with no audio: it fills the transport block the module
// reads, hands over incoming conductor CCs, and posts back whatever the
// module generated. It is the same shape as the bassgen processor, which is
// the other jig:Abi2 generator, and the same two rules apply. Every incoming
// event carries an absolute stream position and is applied in the quantum
// that contains it, compared by range and never by equality with a boundary.
// The queue is bounded, preallocated, and says how much it dropped.
//
...
const PARAM_INDEX = Object.freeze({
  genre: 0, style_mode: 1, channel: 2, kit_map: 3, bars: 4, resolution: 5,
  density: 6, variation: 7, fill_amount: 8, seed: 9, kick_amt: 10,
  backbeat_amt: 11, hat_amt: 12, aux_amt: 13, new: 14, mutate: 15, fill: 16,
  tom_amt: 17, metal_amt: 18, vary: 19, conductor_ch: 20
})
...
class DrumgenProcessor extends AudioWorkletProcessor {
  static get parameterDescriptors () {
    return [
      { name: 'genre', defaultValue: 0, minValue: 0, maxValue: 13, automationRate: 'k-rate' },
      { name: 'style_mode', defaultValue: 0, minValue: 0, maxValue: 6, automationRate: 'k-rate' },
      { name: 'channel', defaultValue: 10, minValue: 1, maxValue: 16, automationRate: 'k-rate' },
      { name: 'kit_map', defaultValue: 0, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'bars', defaultValue: 2, minValue: 1, maxValue: 4, automationRate: 'k-rate' },
      { name: 'resolution', defaultValue: 1, minValue: 0, maxValue: 3, automationRate: 'k-rate' },
      { name: 'density', defaultValue: 0.58, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'variation', defaultValue: 0.35, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'fill_amount', defaultValue: 0.3, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'seed', defaultValue: 1, minValue: 0, maxValue: 65535, automationRate: 'k-rate' },
      { name: 'kick_amt', defaultValue: 0.78, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'backbeat_amt', defaultValue: 0.76, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'hat_amt', defaultValue: 0.82, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'aux_amt', defaultValue: 0.28, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'new', defaultValue: 0, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'mutate', defaultValue: 0, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'fill', defaultValue: 0, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'tom_amt', defaultValue: 0.3, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'metal_amt', defaultValue: 0.26, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
...
registerProcessor('drumgen', DrumgenProcessor)

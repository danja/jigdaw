// plugins/cadence/cadence-processor.js
//
// The AudioWorklet processor for Cadence.
//
// A MIDI harmonizer with no audio: it fills the transport block the module
// reads, hands over incoming notes, and posts back the learned harmony plus
// whatever input passes through. It is the same shape as the drumgen
// processor, which is the other jig:Abi2 plugin with MIDI in both
...
// and tests/host/cadence.test.js fails if the three disagree in a name, a
// range or a default.

const PARAM_INDEX = Object.freeze({
  key: 0, scale: 1, cycle_bars: 2, granularity: 3, complexity: 4,
  movement: 5, chord_size: 6, note_length: 7, register: 8, spread: 9,
  pass_input: 10, output_channel: 11, learn: 12, vary: 13, comp: 14,
  color: 15, arpeggio: 16
})
...
class CadenceProcessor extends AudioWorkletProcessor {
  static get parameterDescriptors () {
    return [
      ...
      { name: 'scale', defaultValue: 3, minValue: 0, maxValue: 23, automationRate: 'k-rate' },
      { name: 'cycle_bars', defaultValue: 2, minValue: 1, maxValue: 8, automationRate: 'k-rate' },
      { name: 'granularity', defaultValue: 1, minValue: 0, maxValue: 2, automationRate: 'k-rate' },
      { name: 'complexity', defaultValue: 0.45, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'movement', defaultValue: 0.65, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'chord_size', defaultValue: 0, minValue: 0, maxValue: 2, automationRate: 'k-rate' },
      { name: 'note_length', defaultValue: 1.0, minValue: 0.1, maxValue: 1, automationRate: 'k-rate' },
      { name: 'register', defaultValue: 1, minValue: 0, maxValue: 2, automationRate: 'k-rate' },
      { name: 'spread', defaultValue: 0, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'pass_input', defaultValue: 1, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'output_channel', defaultValue: 0, minValue: 0, maxValue: 16, automationRate: 'k-rate' },
      ...
      { name: 'color', defaultValue: 0, minValue: 0, maxValue: 1, automationRate: 'k-rate' },
      { name: 'arpeggio', defaultValue: 0, minValue: 0, maxValue: 1, automationRate: 'k-rate' }
    ]
  }
  ...
  process (inputs, outputs, parameters) {
    ...
    // A harmonizer keeps running: input may arrive at any time.
    return true
  }
}

registerProcessor('cadence', CadenceProcessor)
